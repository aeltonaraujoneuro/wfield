function [Vout, regC, T, hemoVar] = cc_SvdHemoCorrect(U, blueV, hemoV, sRate, frameCnt, highCut, smoothBlue)
...
blueVar = zeros(1,size(U,1),'single');
newVar = zeros(1,size(U,1),'single');
ind = 0:500:size(U,1);
for x = 1:length(ind)  
    if x == length(ind)
        ...
        blueVar(ind(x)+1:end) = var(a, [], 2);
        newVar(ind(x)+1:end) = var(a - b .* regC(ind(x)+1:end)', [], 2);
    else
        a = (U(ind(x)+1:ind(x+1),:) * blueV');
        b = (U(ind(x)+1:ind(x+1),:) * hemoV');
        regC(ind(x)+1:ind(x+1)) = nansum(a.*b,2) ./ nansum(b.*b,2);
        blueVar(ind(x)+1:ind(x+1)) = var(a, [], 2);
        newVar(ind(x)+1:ind(x+1)) = var(a - b .* regC(ind(x)+1:ind(x+1))', [], 2);
    end
end
clear a b
...
hemoVar = 100 * (1 - (sum(newVar) ./ sum(blueVar)));
